...
- OpenAI API wrapper using `gpt-4o-mini` by default.
...
- `src/server/openai.ts` owns OpenAI request/response handling and JSON parsing.
...
6. Service calls OpenAI with strict JSON instructions.
...
- Invalid OpenAI JSON returns `502`.

## Testing

Unit tests cover:

- OpenAI prompt/request JSON behavior without real network calls.
...
Integration with the real OpenAI API is not run in CI.
